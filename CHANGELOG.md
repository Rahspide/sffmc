...
- **Утечка памяти в map `this.runs`** — `WorkflowRuntime.close()` теперь очищает `this.runs`; при завершении прогона (complete/fail/cancel) соответствующая запись удаляется. Ранее mcpBridge, journalResults и AbortController накапливались на протяжении всего времени жизни экземпляра runtime. **Исключение из политики**: этот коммит модифицирует `runtime.ts` (запрещено согласно политике hotfix v0.14.1) для устранения утечки памяти C-2, обнаруженной в обзоре W10–W13. Обоснование: исправление хорошо покрыто тестами (5 новых тестов в `v0-14-3-this-runs-cleanup.test.ts`), аудит W10–W13 пометил это как критическую утечку памяти в долгоживущих runtime, а альтернатива (отложить до v0.15) привела бы к отгрузке известной регрессии.
...
- **Чистка документации и комментариев** — номер строки в JSDoc для `recoverOrphanedWorkflows`, описание pre-H5 recovery в `codemap.md`, тестовый комментарий в `w10-w14-hardcode-runtime.test.ts:142`.
- **Несоответствие имени поля phase-события (CRIT-1)** — `schema-journal.ts` валидировал поле `name` у phase-событий, тогда как `runtime.ts:942-946` записывает поле `title` (а `types.ts:57` определяет его как `title`). Каждое phase-событие, записанное runtime, молча отбрасывалось валидатором и пропускалось в `loadJournal`. Валидатор и тип `JournalEventPhase` приведены к использованию `title`. Добавлен регрессионный тест в `v0-14-3-schema-journal.test.ts:160` (phase-событие с pass=3 → maxPass=3 → journal.pass=4).

### Добавлено

- **Рефакторинг схемы M4 (Фаза 1, MVP)** — объявление и валидация схемы событий журнала в `persistence.ts:357-390`. Некорректные события логируются на уровне debug и пропускаются — соответствует существующему поведению для «битых» строк. Закрывает пункт M4 аудита Manriel за 3 часа: 1 новый + 1 правка + 1 тестовый файл. Полная дорожная карта из 5 фаз (27 ч) задокументирована для будущих фаз.
- **Перенос хардкодов, Фаза 2** — 21 хардкод MEDIUM-уровня перенесён в индивидуальные `~/.config/sffmc/{package}.yaml` для каждого пакета. Значения по умолчанию точно соответствуют поведению v0.14.2.
...
- 4 подагентских исследования (M4, Hardcode, W10–W13, Housekeeping) → сводный план
- Совет (multi-LLM синтез) проверил сводный план перед реализацией
- 740+ pass + 1 skip + 0 fail (было 722 в v0.14.2)

## v0.14.2 (2026-06-20)

Закрытие аудита Manriel (все 30 пунктов) + исправление flushNow NOT NULL. См. [`README.md`](./README.md) для подробностей. 2 коммита с v0.14.1.

## v0.14.1 (2026-06-19)

Hotfix: наблюдаемость автоматического cap'а + редактирование тела PEM + ReDoS-шлюз CI + исправление ссылки w5-6. 4 коммита с v0.14.0.
...
- **M5.2 Редактирование тела PEM-ключа** (`shared/src/redact-secrets.ts`) — регулярное выражение для PEM расширено до полного блока (заголовок + тело + окончание). Ранее редактировался только заголовок `-----BEGIN ... PRIVATE KEY-----`; теперь редактируется и base64-кодированное тело ключа. 7 новых тестов (#29–35).
- **Устаревшая ссылка w5-6 dynamic-workflow** (`README.md`) — исправлена битая ссылка из v0.14.0.

### Не изменялось (уже в порядке)

- Файлы верхнего уровня `README.md`, `AGENTS.md`, `CONTRIBUTING.md` — проверены: не содержат жаргона F-кодов / W-кодов / DLC / Phase / Drop-in Lattice. Ранние полирующие коммиты v0.14.0 (`b7faec7`, `64a3390`) уже закрыли этот объём работ.
- Файлы `**/codemap.md` — остаются в `.gitignore` (codemap — внутренний/генерируемый документ, не источник истины). Локальные правки fix-7 сохранены на диске как артефакты.

### Проверка

- 680 pass / 1 skip / 0 fail тестов (+12 с v0.14.0)
- 6 шлюзов precommit зелёные (было 5)
- Все 4 отложенных пункта v0.12.1 без изменений по статусу (M5/M6 ✅, H5 ✅, L1/L2 ✅; M2 ❌ не начат; M4 — только дизайн)

---

## v0.14.0 (2026-06-19)

Хелпер редактирования секретов + grace-период + MCP-интеграция + повторный полировщик I-1. 5 коммитов с v0.12.1.

### Добавлено

- **M5/M6 Общий хелпер редактирования секретов** (`shared/src/redact-secrets.ts`, 240 LOC) — три чистые функции (`isSensitiveFilename`, `isSensitiveSourcePath`, `redactSecrets`) + 15 встроенных правил в 4 категориях (env-файлы, имена файлов с секретами, PEM-ключи, инлайн-присваивания). Настраивается через `~/.config/sffmc/redact-secrets.yaml`. Закрывает проблему чрезмерно широких регулярных выражений из аудита Manriel (например, `token` срабатывал на `tokendeploy.sh`, `private` — на `private-blog.md`).
- **MCP INHERIT-интеграция** (`packages/workflow/src/mcp.ts`, 298 LOC) — скрипты workflow могут вызывать MCP-инструменты, унаследованные от родительской сессии. Два интерфейса: `agent({task, tools: "INHERIT"})` резолвит MCP-инструменты родителя и передаёт LLM как конкретный массив; гостевые глобалы `mcp.list()` и `mcp.call(name, args)` для прямого вызова MCP. Per-run `McpBridge` с бюджетом (`DEFAULT_MAX_MCP_CALLS=500`) + защитой от рекурсии (`RECURSION_DEPTH_LIMIT=8`).
- **Повторный полировщик документации I-1** (`коммит 312039f`) — восстановлены потерянные правки удаления жаргона из потерянного коммита `f9a42be`. Применено выборочно к 13 README пакетов + `docs/install.md` + `packages/memory/skills/recall.md`. Удалены коды F1-F8 / W1-W8, теги `(MiMo)`, числовые ссылки на фазы (Phase N).

### Изменено

- **Хук grace-периода H5** (`packages/workflow/src/constants.ts`, `runtime.ts`, `types.ts`) — при перезапуске OpenCode workflow в состоянии `running` с возрастом ≤ `gracePeriodMs` помечаются как `paused` (возобновляемые); более старые проходят проверку наличия записи в журнале. Значение по умолчанию `gracePeriodMs = 5 минут`, потолок `MAX_GRACE_PERIOD_MS = 24 часа`. Настраивается через `~/.config/sffmc/workflow.yaml`. Поле в `WorkflowConfig`.
- **Сужение L1/L2 регулярных выражений** — `packages/memory/src/watcher.ts` и `recon.ts` теперь вызывают общие хелперы из `redact-secrets.ts` вместо дублирующихся deny-листов на 7 регулярок. Чувствительные имена файлов привязаны к `basename()`; чувствительные source-пути используют и basename-, и path-уровневые правила.
...
665 → 664 pass / 1 skip / 0 fail (один H5-тест пропущен из-за специфического асинхронного тайминга окружения). +95 тестов всего с v0.12.0 (570 → 664).

### Отложено в v0.15

- **M2** изменение формата checkpoint (отложено с v0.12.1, не перепланировано)
- **M4** рефакторинг схемы (дизайн в `v0-14-m4-schema-design.md`, 990 строк; реализация 21–31 ч → v0.15)
- Результаты аудита хардкодов (60 находок: 33 HIGH, 15 MEDIUM, 12 LOW — см. `.slim/deepwork/hardcode-audit-2026-06.md`)
- M5.2 редактирование тела PEM (вне scope v0.14)
...
Исправления аудита безопасности — 30 hardening-коммитов от внешнего контрибьютора Manriel.

### Исправлено

- **H1 — Jail от path traversal для файлов workflow** (`packages/workflow/src/runtime.ts`): `resolveWorkflow()` теперь отклоняет пути, выходящие за пределы корня рабочего каталога. Тесты покрывают `../`, `/etc/passwd` и смешанные случаи `./dir/../../etc`.
- **H2 — Jail от path traversal для `input.file`** (`packages/workflow/src/runtime.ts:450-458`): та же защита для поля workflow `input.file`.
- **H3 — Git-токен в URL** (`packages/workflow/src/resolve.ts`): токены перенесены из URL в `http.extraHeader`.
- **H4 — Проверка GPG-подписи** после clone/pull; строгий GPG-режим.
- **H5 — Дедлайн песочницы сокращён** с 12 ч до 1 ч wall-clock.
- **H6 — Параллельные LLM-кандидаты ограничены** 10, чтобы предотвратить злоупотребление API.
- **H7 — `JSON.parse` обёрнут в try/catch** для повреждённых данных БД.
- **C1 — Дедуп-записи Dream ограничены** для предотвращения O(n²)-взрыва.
- **C2 — LRU буфер сессий checkpoint** (`packages/extra/src/checkpoint.ts`): настоящее LRU-вытеснение через `delete + re-set` на каждом попадании (был FIFO).
- **C3 — Единообразные предупреждения oversize**: `readHeader` и `readToolCalls` теперь логируют идентичные сообщения `checkpoint: skipping … exceeds limit`.
- **C4 — Слишком большой AGENTS.md отклоняется** до чтения в память.
- **M1 — YAML-парсинг использует `Schema.JSON`** в пакете rules.
- **M3 — Резолвинг дочернего workflow использует рабочий каталог родителя**.
- **M7 — Ограничительные права доступа к каталогам данных**.
- **M10 — Восстановленные из checkpoint сообщения ограничены 50**.
- **L1 — Чувствительные имена файлов пропускаются** при индексировании памяти.
- **L2 — Чувствительные source-пути фильтруются** из LLM-recon-инъекции.
- **L3 — Event bus логирует только сообщение об ошибке**, а не весь объект ошибки.
- **L4 — Нарушение DLC в `panicMode` задокументировано** + добавлен `resetPanicMode()`.
- **L6 — TOCTOU-гонка в `WorkspaceJail` задокументирована**.
- **L7 — `WORKFLOW_LIMITS` валидируются** до SQL DDL-интерполяции.
- **L9 — Ошибки legacy-миграции логируются как warning** вместо молчаливого проглатывания.
...
- Сужение L1/L2 регулярных выражений (слишком широкий scope).
- M2 изменение формата checkpoint.
- M4 рефакторинг схемы, M5/M6 объединённый хелпер редактирования.
- H5 grace-период 12 ч → 1 ч (риск регрессии; требует хука в AGENTS.md).
...
- **Dream**: именование кластеров через LLM — `nameClusterViaLLM` генерирует 3–5-словную фразу-тему
...
- LLM-judge скорит 2–8 кандидатов-выходов
...
- LLM-вызов с температурой 0.2 для детерминизма
...
- Кластерная суммаризация: Jaccard > 0.3 кластера, 5+ записей → LLM-резюме
- Конкурентность: Promise-лок предотвращает перекрывающиеся запуски
- LLM-суммаризация с корректным фоллбэком на конкатенацию при ошибке
...
```yaml
checkpoint: true      # capture + restore
judge: true           # мульти-критериальный LLM-скоринг
...
- `@sffmc/health` теперь загружается в dev-песочнице — LLM может вызывать инструмент `sffmc_health` в сессиях песочницы
...
- Экспонирует один вызываемый из LLM инструмент `sffmc_health`, возвращающий JSON.
...
- `tdd` — 5-фазная генерация TDD-артефактов (Spec → Red → Green → Refactor → Verify). Принимает `args.feature`, возвращает тестовый файл + файл реализации + заметки по рефакторингу как артефакты. Генерирует, НЕ выполняет (только LLM).
...
- 5-слойный бюджет: lifecycle 1000, concurrent 16, depth 8, wall-clock 12 ч, token 2M
...
- Plan, TDD, verify, subagent и ещё 11 структурированных workflow
