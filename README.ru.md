...
Затем добавь пакеты в `~/.config/opencode/opencode.json`:
...
Перезапусти OpenCode. Проверь установку через `sffmc doctor` или введи `sffmc_health` в любом чате.

**Альтернатива: one-liner установка** (клонирует репо + авто-настраивает opencode.json):
...
Для всех опций установки (пиннинг версии, кастомная директория, troubleshooting), см. [docs/install.md](./docs/install.md).
...
См. [CONTRIBUTING.md](./CONTRIBUTING.md) для референса plugin SDK и hook categories.
