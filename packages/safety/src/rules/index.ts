...
rules:
  ...
  - match:
      ...
      command_match: "^rm\\\\s+-r[f]?\\\\s+/\\\\*"
    action: deny
  # v0.15.2 final polish: allow rules for cases that bundled ASK wrongly
  # flags (chmod benign octals, dd file-to-file, git --force-with-lease,
  # sudo -k). Fire BEFORE bundled ASK so fallback never sees the case.
  - match:
      tool: bash
  ...
  - match:
    ...
    action: allow
  # Case-insensitive mkfs / chmod 777 (NFKC folds fullwidth to ASCII but
  # preserves case, so we still need explicit case folding here).
  - match:
      tool: bash
  ...
  # The bundled ASK rule's "dd if=" substring over-triggers on benign dd
  # usage (e.g. dd if=/foo of=/bar for file-to-file copy).
  - match:
      tool: bash
