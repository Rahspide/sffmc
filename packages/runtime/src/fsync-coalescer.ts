...
/** Callback type for fsync errors. Aliased from `unknown` so the parameter
 *  type has a domain name (satisfies the no-unknown-parameters rule). */
// oxlint-disable-next-line no-unknown-type-aliases
export type FsyncErrorValue = unknown
