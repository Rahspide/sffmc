...
// Per-run MCP bridge — budget + recursion guard + host primitives.
//
// Extracted from mcp.ts so the McpBridge class and the makeMcpPrimitives
// factory (which injects `mcp.list/call/bind/bindAll` into the guest sandbox)
// live together. Both share the per-run state that lives on McpBridge.
//
 ...
 *  per-run; the `dispatch` callback is what actually invokes the MCP tool —
 *  wired in by the runtime so the bridge stays SDK-agnostic (testable in
 *  isolation).
 *
 *  Returned shape:
