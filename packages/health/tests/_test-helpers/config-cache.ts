...
// only purpose is to give tests a single import path for the health
// config surface (v0.14.3 D-1).
//
// The shim pulls most symbols through normal `export ... from`
// re-exports, plus a Symbol-registry indirection for __setHealthConfig
// (which is no longer publicly exported from src/index.ts). This means:
//   - src/index.ts does NOT add a public export of __setHealthConfig
//   - this shim DOES export __setHealthConfig as a callable function
  ...
  if (!fn) {
    throw new Error(
      "__setHealthConfig: src/index.ts was not loaded before this test " +
        "helper. Import something from ../../src/index.ts in your test file " +
