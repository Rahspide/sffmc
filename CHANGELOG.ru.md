...
Все значимые изменения SFFMC документируются здесь. Даты в формате `YYYY-MM-DD`.

## v0.16.5 (2026-08-22)

> Patch-релиз. **Без ломающих изменений.** Security-фикс в `@sffmc/safety`: два deny-правила
> с момента создания не срабатывали ни на одном вводе — теперь работают как задумано.

### Исправлено

- **@sffmc/safety: восстановлены два молча неработавших deny-правила.** Формы с quoted-HOME
  (`rm -rf "$HOME"`, `rm -rf "${HOME}"`) и quoted-root-slash (`rm -rf "/"`, `rm -rf "//"`)
  не матчили никакой ввод: их паттерны заданы внутри JS template literal, где одинарные
  бэкслеш-эскейпы отбрасываются до того, как их увидит regex-движок. Эскейпы удвоены —
  скомпилированные паттерны теперь матчат whitespace и литеральный доллар как задумано.
- **@sffmc/runtime: разорван type-only цикл импортов.** `SandboxPrimitives` перенесён
  в листовый модуль (`sandbox-types.ts`); направление импортов строго однонаправленное.
- Устранены 16 нарушений `no-unknown-parameters` через доменные алиасы на границах
  (catch-колбэки, параметры колбэков) в src и тестах. Изменений поведения нет.

### Изменено

- Дефолтный YAML правил safety извлечён из `rules/index.ts` в `rules/default-rules.ts`;
  модуль сократился с 605 до 116 строк.

### Статистика

- oxlint anti-slop аудит: 28 → 0 нарушений во всех 5 пакетах
- @sffmc/safety: 706 тестов проходят
...
- **Редизайн README (английский + русский)** -- новая главная страница с
  центрированным логотипом, TL;DR-блоком установки, сравнительной таблицей,
  таблицей пакетов с per-package npm-бейджами, тремя вариантами установки
  (npm / one-liner / из исходников), CLI-справочником, диаграммой архитектуры,
  демо-gif safety-gate, таблицей quality-гейтов и quickstart-ом для контрибьюторов.
  Новые брендовые ассеты в `docs/assets/`: `logo.svg`, `logo-mark.svg`,
...
- Количество тестов: 1946 тестов в 109 файлах (per-file runner).
...
> Patch-релиз. **Ломающих изменений нет.** Усиление движка `@sffmc/safety` с полным покрытием корпуса; исправление v1-миграции чекпоинтов в `@sffmc/memory`.

### Исправлено

- **Движок правил `@sffmc/safety`** — двухфазное сопоставление: `phase: raw` для обфускационных эвристик (ANSI-escape, NUL-байты) и `phase: normalized` (по умолчанию) для определения execution-shape. Полный базовый корпус (`packages/safety/test/corpus.bash`, 281 кейс) теперь проходит с нулём ошибок. Якорные позиции на `;`, `&&`, `||`, `|`, `$(`, backtick и обёртках (`sudo`, `env`, `exec`, `nohup`, `setsid`, `time`, `bash -c`, `sh -c`) предотвращают ложные deny на данных внутри аргументов (commit messages, `grep`-паттерны, `echo`-данные).
- **Миграция чекпоинтов `@sffmc/memory`** — `readV1BodyLines` (v1→v2-хелпер) падал с `ReferenceError: i is not defined` на первой битой JSON-строке. Оба пути — skip-bad-line и auto-migrate — теперь корректно пропускают битую строку, сохраняя хорошие.

### Добавлено

- **`@sffmc/safety`** — опция `excludeSubstitutions` в `commandWordPositions()` / `anchoredTest()` позволяет обфускационным правилам якориться только на верхнеуровневые команды. Та же форма внутри `$(...)` и backtick — обычно инертные данные (например, `echo $(printf "\x1b[31mrm -rf /\x1b[0m")`), и опция предотвращает ложные deny. Реальное выполнение подстановок всё равно ловится normalized-фазой через content-substitution pass и специальными executor-правилами (`^\$\(`, `^eval\s+\$\(`, `^env\s+[^\n]*\$\(`).
- **Покрытие тестами** — `packages/safety/test/expanded-coverage.test.ts` добавляет 64 теста в 13 секциях: phase-система, `excludeSubstitutions`, edge-кейсы рекурсии подстановок (глубоко вложенные `$($(...))`, незакрытые `$(`, сбалансированные backtick, смешанные backtick + `$()`), fail-closed поведение, жизненный цикл panic-mode, mtime-reload `watchRules`.

### Статистика

- Изменено 16 файлов.
- Все 5 пакетов на `0.16.2`; root на `0.16.2`.
- Тесты monorepo: 1951 тестов в 110 файлах / 0 fail.

### Благодарности

- Спасибо **Professor_Sovers** за аудит безопасности, который выявил оставшиеся ошибки в корпусе и определил архитектуру двухфазного сопоставления правил.
...
- **`mcp.bind(name)` и `mcp.bindAll()`** - скрипты workflow теперь могут получить типизированный хэндл к одному MCP-инструменту (`const search = mcp.bind("github_search")`) или сразу ко всем доступным (`const { a, b } = await mcp.bindAll()`) вместо поиска по имени на каждом вызове. Привязанные функции проходят через тот же bridge, поэтому лимиты и защита от рекурсии продолжают работать.
- **`startWorkflowWatcher(workspace)`** - обёртка над `node:fs.watch` для каждой сконфигурированной workflow-поддиректории (`.sffmc/workflows/`, `.claude/workflows/`, подъём по дереву). Эмитит событие `workflow:file-changed` для add/change/remove файлов с расширениями `.ts`/`.js`/`.mjs`/`.cjs`. Встроенные потребители могут подписаться для инвалидации кэшей или остановки затронутых запусков; сам watcher не решает, что делать.
- **`redact-secrets.yaml` принимает `minTokenLength`** - целое в `[4, 256]`, по умолчанию `16`. Управляет порогом для `api-key-assignment`, `token-assignment` и `bearer-header`. Вне диапазона - предупреждение и возврат к умолчанию.

### Исправлено

- **Санитизация паттернов в `WorkspaceJail.glob()`** - паттерны, которые после нормализации `path.resolve` выходят за пределы workspace, теперь выбрасывают исключение на границе (например `../*.ts`, `foo/../../bar`, `../../etc/passwd`) вместо тихой фильтрации через realpath-проверку каждого совпадения. Пустые паттерны также выбрасывают исключение. Паттерны, резолвящиеся внутри workspace, продолжают работать как раньше, включая `**/*.md` и `subdir/*`.
